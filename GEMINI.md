...
*   **Root Layout Class Wrapping (Including Shared Library Files)**: Ensure that the very first HTML element in each template file (including shared template files like `shared.liquid` / `markup_shared`) is a layout container class (e.g. `<div class="layout layout--col">`).
