...
*   **Root Layout Class Wrapping (Including Shared Library Files)**: Ensure that the very first HTML element in each template file (including shared template files like `shared.liquid` / `markup_shared`) is a layout container class (e.g. `<div class="layout layout--col">`). Rendered components like `{% render 'title_bar' %}` must be placed inside this container, rather than before it, so the compiler immediately registers a layout class as the root of the file.
*   **No `currentColor` inside SVGs**: Use explicit `#000` colors for SVG `stroke` and `fill` attributes. Do not use `currentColor` as it makes rendering unpredictable on 1-bit screens.
